...
def pdb_to_bins(bin_size , *pdb_list_to_bins):
    ...
    count_of_x_strips = int(round(((x_rang*angstrom2nm) / bin_size) + 0.5))+1 #we put 5 bins more to have some surroundings around molecule
    count_of_y_strips = int(round(((y_rang*angstrom2nm) / bin_size) + 0.5))+1
    ...
    for k in range(0,len(pdb_000)): #iterate trough all atoms
        x_integerized = int((pdb_000[k][0]*angstrom2nm)/bin_size) # divide x coordinate by bin size and round it to lower number 
        y_integerized = int((pdb_000[k][1]*angstrom2nm)/bin_size) # - int function just tears numbers after decimal point
